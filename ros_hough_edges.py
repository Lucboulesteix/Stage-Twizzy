...
def run_detection(): #starting loop, called from app manager (main)
    ...
    hough_pub = rospy.Publisher('/cam_p/color/Hough_projection', Image , queue_size=10) #plane space data
    img_pub = rospy.Publisher('/cam_p/color/ground_detection', Image , queue_size=10) #final (RGB video) image
    try:
...
def callback(lidar,image):
    ...
    img_edge = cv2.Canny(gray, 20,50)

    lines = cv2.HoughLines(img_edge, 1, np.pi/180, 200)
    full = cv2.cvtColor(img_edge, cv2.COLOR_BGR2RGB)
    try:
        for k in lines:
            for rho, theta in k:
                a = np.cos(theta)
                ...
                cv2.line(full, (x1,y1),(x2,y2), (0,0,255),2)
    except:
        pass

    #full = cv2.cvtColor(img_edge, cv2.COLOR_BGR2RGB)
    img = CvBridge().cv2_to_imgmsg(full, "bgr8")
    img_pub.publish(img) #publish vide image
...
def callback2(lidar, image): #routine when new synchronised data is found
